...
HIGHLY OPTIMIZED VERSION WITH MULTI-CORE FIXES:

- Blosc configured for multi-threaded compression (uses all cores)
- TensorStore context with increased file_io_concurrency
- Larger default max_concurrent_writes (8 instead of 4)
- ProcessPoolExecutor for CPU-intensive PFF parsing
...
# This is critical for utilizing multiple cores during compression
try:
    import blosc
    # Set blosc to use multiple threads (up to system max)
    num_cores = os.cpu_count() or 4
    blosc.set_nthreads(num_cores)
    print(f"Blosc configured to use {num_cores} threads for compression")
except ImportError:
    print("Warning: python-blosc not installed, using tensorstore's blosc (may be single-threaded)")
    print("Install with: pip install blosc")
...
async def _open_ts_array(root_path, name, shape, chunks, np_dtype, codec_chain,
    ...
    """Open TensorStore array with absolute path resolution and context for concurrency"""
    ...
    # Use context if provided for concurrency control
    if context:
...
async def convert_pff_to_tensorstore(
    ...
    level: int = 1,  # Lower compression level for more speed (was 3)
    time_chunk: int = 131072,  # EXTRA LARGE chunks for speed (was 65536)
    max_concurrent_writes: int = 8,  # INCREASED from 4 to 8
):
    """
    Convert PFF to Zarr with optimizations for large files:
    - Large chunks reduce write operations
    - Blosc-LZ4 with multi-threading uses all CPU cores
    - TensorStore context configured for high concurrency
    ...
    # Create TensorStore context with increased concurrency
    # This is KEY to multi-core utilization
    num_cores = os.cpu_count() or 4
    context = ts.Context({
        'file_io_concurrency': {'limit': max(num_cores, max_concurrent_writes * 2)},
        'cache_pool': {'total_bytes_limit': 1_000_000_000}  # 1GB cache
    })
    print(f"TensorStore context: file_io_concurrency={max(num_cores, max_concurrent_writes * 2)}")

    # Create TensorStore arrays with context
    ...
    # Progress bar
    pbar = tqdm(total=T, desc="Converting PFF -> Zarr (Multi-Core Optimized)", unit="frames")
...
async def main():
    if len(sys.argv) != 3:
        print("Usage: python step1_pff_to_zarr.py <input.pff> <output.zarr>")
        print()
        print("Environment variables for tuning:")
        print("  TS_CODEC=blosc-lz4|zstd|gzip|none (default: blosc-lz4)")
        print("  TS_LEVEL=1-9 (default: 1, optimized for speed)")
        print("  TS_CHUNK=131072 (default: 131072 frames, doubled for speed)")
        print("  TS_CONCURRENT=8 (default: 8 parallel writes, doubled)")
        print()
        print("MULTI-CORE OPTIMIZATION:")
        print("  - Blosc now uses all CPU cores for compression")
        print("  - TensorStore file_io_concurrency increased")
        print("  - Default concurrent writes increased from 4 to 8")
        sys.exit(1)

    pff_path = sys.argv[1]
    zarr_root = sys.argv[2]

    # Read tuning parameters from environment with NEW DEFAULTS
    codec = os.environ.get("TS_CODEC", "blosc-lz4")
    level = int(os.environ.get("TS_LEVEL", "1"))  # Changed from 3 to 1 for speed
    time_chunk = int(os.environ.get("TS_CHUNK", "131072"))  # Doubled from 65536
    max_concurrent = int(os.environ.get("TS_CONCURRENT", "8"))  # Doubled from 4
